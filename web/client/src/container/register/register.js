...
class Register extends React.Component {
  ...
  handleRegister() {
    // const user = this.state.user; const pwd = this.state.pwd; const type =
    // this.state.type; axios.post('/users/register', {user, pwd, type}) .then(res
    // => { 	if (res.status === 200 && res.data.code === 0) { 	}else{ 	} });
    ...
    return (
      <div>
        ...
        <List>
          ...
          <WhiteSpace/>
          <InputItem type="password" onChange={v => this.handleChange('repeatpwd', v)}>Again</InputItem>
          <WhiteSpace/>
          <RadioItem
          ...
          <WhiteSpace/>
          <Button type='primary' onClick={this.handleRegister}>Resiger
          </Button>
        </List>
      </div>
